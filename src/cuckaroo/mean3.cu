...
#define SEG 2
const u32 EDGES_A = NZ * NEPS_A / NEPS / SEG;
const u32 EDGES_B = NZ * NEPS_B / NEPS / SEG;
...
__global__ void SeedA(const int round, const int seg, const siphash_keys &sipkeys, ulonglong4 * __restrict__ buffer, u32 * __restrict__ indexes, u64* one_bit_set) {
  ...
  for (int blk = 0; blk < loops; blk += EDGE_BLOCK_SIZE) {
    ...
    for (u32 e = 0; e < EDGE_BLOCK_SIZE; e++) {
      ...
      tmp[row][counter] = make_uint2(node0, nonce);//make_uint2(node0, node1);
      __syncthreads();
      if (counter == FLUSHA-1) {
...
template<int maxIn>
__global__ void Round(const int round, const uint2 * __restrict__ src, const u32 * __restrict__ srcIdx, u32 * two_bit_set, u64* one_bit_set) {
  const int group = blockIdx.x;
  const int dim = blockDim.x;
  const int lid = threadIdx.x;
  const int COUNTERWORDS = NZ / 16; // 16 2-bit counters per 32-bit word
  ...
  for (int loop = 0; loop < loops; loop++) {
      ...
      if (lindex < edgesInBucket) {
          ...
          if (0 == Read2bCounter(ecounters, node0 & ZMASK)) {
              ...
              int bit_set_index1 = nonce >> 6;
              int bit_set_index2 = nonce & 63;
              int bit_set_v = ~(1 << bit_set_index2);
...
struct edgetrimmer {
  ...
  u32 trim() {
    ...
    cudaMemset(one_bit_set, 0XFFFFFFFF, NX2 * NZ / 8);
    int rounds = 20;
