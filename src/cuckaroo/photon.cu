...
#ifndef NEPS_A
#define NEPS_A 150 // to match Photon's kernel.cu
#endif
#ifndef NEPS_B
#define NEPS_B 84 // to match Photon's kernel.cu
#endif
#define NEPS 128
...
struct edgetrimmer {
  ...
  void indexcount(u32 round, const u32 *indexes, size_t size) {
#ifdef VERBOSE
    u32 nedges = 0;
    u32 n = size / sizeof(u32);
    u32 *hindex = new u32[n];
    cudaMemcpy(hindex, indexes, size, cudaMemcpyDeviceToHost);
    for(int i = 0; i < n; i++){
        nedges += hindex[i]; 
    }
  ...
  u32 trim() {
    ...
    for (u32 i=0; i < NA; i++) {
      FluffySeed<SEED_TPB, EDGES_A/NA><<<tp.seed.blocks, SEED_TPB>>>
          ((uint4*)(bufferA+i*(sizeA/NA2)), indexesA+i*NX2, i*(NEDGES/NA));
...
#ifdef VERBOSE
    print_log("%EDGESeed<<<%d,%d>>>\n", NA, tp.seed.blocks, tp.seed.tpb); // 1024x512
    indexcount(0, indexesA, indexesSizeNA);
   // uint2 *buf = new uint2[EDGES_A/NA];
   // u32 *hindex = new u32[NX2*NA];
   // cudaMemcpy(hindex, indexesA, indexesSizeNA, cudaMemcpyDeviceToHost);
   // for(int i = 0; i < NA; i++){
   //   uint2 *dbuf = (uint2*)(bufferA + i * (sizeA/NA2));
   //   for(int j = 0; j < NX2; j++){
   //     int offset = j/NA * (sizeA/NA);
   //     int n = hindex[j] > EDGES_A/NA ? EDGES_A/NA : hindex[j];
   //     cudaMemcpy(buf, dbuf + offset + j * (EDGES_A/NA), n*sizeof(uint2), cudaMemcpyDeviceToHost);
   //     qsort(buf, n, sizeof(uint2), compare);
   //     for(int ii = 0; ii < n-1; ii++){
   //       if(buf[ii].x != 0 && buf[ii].x == buf[ii+1].x && buf[ii].y == buf[ii+1].y){
   //         printf("find same edges on seed : %u %u %u\n", i, buf[ii].x, buf[ii].y);
   //         return 0;
   //       }
   //     }
   //   }
   // }
    ...
    const u32 qI = NX2 / NA;
    for (u32 i=0; i < NA; i++) {
      FluffyRound_A1<TRIM0_TPB, EDGES_A/NA, EDGES_B/NA><<<NX2/NA, TRIM0_TPB>>>
        ((uint2*)bufferA, (uint2*)(bufferB+i*qB), indexesA, indexesB, i*qI);
      cudaDeviceSynchronize();
      if (abort) return false;
    }
...
#ifdef VERBOSE
    indexcount(1, indexesB, indexesSizeNA);
    print_log("Round A1 completed in %.0f ms\n", durationB);
    print_log("Round_A3<<<%d,%d>>>\n", NX2/NA, TRIM1_TPB); // 4096x1024
#endif

    cudaMemset(indexesA, 0, indexesSize);
    FluffyRound_A3<TRIM1_TPB, EDGES_B/NA, EDGES_B/2><<<NX2, TRIM1_TPB>>>
      ((uint2*)bufferB, (uint2*)bufferA1, indexesB, indexesA);
    indexcount(2, indexesA, indexesSize);
    if (abort) return false;
    ...
    FluffyRound_A2<TRIM_TPB, EDGES_B/2, EDGES_A/4><<<NX2, TRIM_TPB>>>
        ((uint2*)bufferA1, (uint2*)bufferB, indexesA, indexesB);
    indexcount(3, indexesB, indexesSize);
    if (abort) return false;

    cudaMemset(indexesA, 0, indexesSize);
    FluffyRound_A2<TRIM_TPB, EDGES_A/4, EDGES_B/4><<<NX2, TRIM_TPB>>>
        ((uint2*)bufferB, (uint2*)bufferA1, indexesB, indexesA);
    indexcount(4, indexesA, indexesSize);
    if (abort) return false;

    cudaMemset(indexesB, 0, indexesSize);
    FluffyRound_A2<TRIM_TPB, EDGES_B/4, EDGES_A/8><<<NX2, TRIM_TPB>>>
        ((uint2*)bufferA1, (uint2*)bufferB, indexesA, indexesB);
    indexcount(5, indexesB, indexesSize);
    if (abort) return false;

    cudaMemset(indexesA, 0, indexesSize);
    FluffyRound_A2<TRIM_TPB, EDGES_A/8, EDGES_B/8><<<NX2, TRIM_TPB>>>
        ((uint2*)bufferB, (uint2*)bufferA1, indexesB, indexesA);
    indexcount(6, indexesA, indexesSize);
    if (abort) return false;

    for (int round = 7; round < tp.ntrims; round += 2) {
      cudaMemset(indexesB, 0, indexesSize);
      FluffyRound_A2<TRIM_TPB, EDGES_B/8, EDGES_B/8><<<NX2, TRIM_TPB>>>
          ((uint2*)bufferA1, (uint2*)bufferB, indexesA, indexesB);
      indexcount(round, indexesA, indexesSize);
      if (abort) return false;

      cudaMemset(indexesA, 0, indexesSize);
      FluffyRound_A2<TRIM_TPB, EDGES_B/8, EDGES_B/8><<<NX2, TRIM_TPB>>>
          ((uint2*)bufferB, (uint2*)bufferA1, indexesB, indexesA);
      indexcount(round+1, indexesB, indexesSize);
      if (abort) return false;
    }
    ...
    print_log("Tail<><<<%d,%d>>>\n", NX2, TAIL_TPB);
#endif
    Tail<EDGES_B/8><<<NX2, TAIL_TPB>>>((const uint2 *)bufferA1, (uint2 *)bufferB, indexesA, indexesB);
...
struct solver_ctx {
  ...
  int findcycles(uint2 *edges, u32 nedges) {
    ...
    for (u32 s = 0 ;s < cg.nsols; s++) {
      ...
      cudaError_t err = cudaMemcpyToSymbol(recovery, soledges, sizeof(soledges));
